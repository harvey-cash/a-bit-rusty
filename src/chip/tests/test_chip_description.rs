...
fn given_no_outputs_then_invalid() {
    let description = ChipDescription::new(1, 0, 0, vec![Link::new(0, 0)]);
	assert_eq!(description.is_valid(), false);
}
...
fn given_link_source_out_of_range_then_invalid() {
    let description = ChipDescription::new(1, 0, 1, vec![Link::new(5, 0)]);
    assert_eq!(description.is_valid(), false);
...
fn given_any_node_unconnected_then_invalid() {
    let description = ChipDescription::new(1, 0, 2, vec![Link::new(0, 1)]);
    assert_eq!(description.is_valid(), false);
...
fn given_nand_no_sources_then_invalid() {
    let description = ChipDescription::new(1, 1, 2, vec![Link::new(0, 2), Link::new(1, 3)]);
	assert_eq!(description.is_valid(), false);
}
