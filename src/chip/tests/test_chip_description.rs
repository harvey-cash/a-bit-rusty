...
fn given_no_links_then_invalid() {
    let description = ChipDescription::new(1, 1, 0, vec![]);
	assert_eq!(description.is_valid(), false);
}
...
fn given_single_wired_up_nand_then_valid() {
    ...
    let description = ChipDescription::new(
        2, 1, 1, 
        vec![Link::new(input_1, nand), Link::new(input_2, nand), Link::new(nand, output)]
    );
    assert_eq!(description.is_valid(), true);
}
...
fn given_link_source_out_of_range_then_invalid() {
    let description = ChipDescription::new(1, 1, 0, vec![Link::new(7, 2)]);
    assert_eq!(description.is_valid(), false);

    let links = vec![Link::new(2, 2), Link::new(7, 2)];
    let description = ChipDescription::new(1, 1, 0, links);
    assert_eq!(description.is_valid(), false);
}
...
fn given_output_targeted_by_two_links_then_invalid() {
    ...
    let description = ChipDescription::new(2, 1, 0, links);
	assert_eq!(description.is_valid(), false);
}
...
fn given_any_node_unconnected_then_invalid() {
    ...
    let description = ChipDescription::new(1, 0, 2, vec![Link::new(input, nand)]);
    assert_eq!(description.is_valid(), false);
...
fn given_nand_no_sources_then_invalid() {
    ...
    let description = ChipDescription::new(1, 2, 1, vec![Link::new(input, output_1), Link::new(nand, output_2)]);
	assert_eq!(description.is_valid(), false);
}
